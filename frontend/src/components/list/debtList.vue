        ...
        <section id="table_section">
            <div id="table_wrapper">
                <table>
                    ...
                    <tbody>
                        <tr v-for="list in debtLists" :key="list.sid" v-on:click="visibleTools(list.sid)" :ref="'tr'+list.sid">
                            ...
                            <td v-if="toolState.state.modify && list.sid === toolState.id"><input v-model=modifyForm.amount v-on:keyup.enter="clickTools($event, list)"></td>
                            <td v-else>{{list.amount}}</td>
...
export default {
    ...
    data(){
        return{
            debtLists:[
               {sid:1, date:"1", usage:1, detail:"1", user:"11", amount:"1"},
            ],
            clickedElement: null,
            toolState:{
                id:-1,
    ...
    methods:{
        clickAddUser(event){
            ...
            this.$set(this.debtLists, this.debtLists.length, 
            {
                sid : tempId,
                date : "",
                usage: "",
        ...
        clickTools(event, list){
            ...
            //테이블 삭제
                ...
                if(className.search("true") !== -1){
                    for(let index in this.debtLists){
                        if(this.debtLists[index].sid === list.sid){
                            ...
                            setTimeout(() => {
                                this.$delete(this.debtLists, index);
                            }, 500);
